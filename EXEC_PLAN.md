...
- Native macOS arm64 Bazel 9.2.0 static/shared Crashpad consumers pass after the same header split. Bazel 8.4.2 passed the static consumer; one first shared run segfaulted during SDK shutdown, then three uncached reruns passed. Treat the isolated first failure as a recorded runtime flake, not positive evidence, while retaining the BCR native macOS lane as the publication gate.
