...
- Breakpad source/header groups now live in `bazel/breakpad_sources.bzl`, parallel to the Sentry and Crashpad manifests; the root `BUILD.bazel` retains only the Breakpad target declarations and platform selection.
- Repository/publication hardening is implemented: every Bazel-owned Breakpad/Crashpad file lives outside Git-submodule paths. All ordinary target declarations appear explicitly in the superproject's root `BUILD.bazel`; `.bzl` files retain only source manifests, pure attribute helpers, and genuine custom rule implementations. The documented public labels are unchanged. The uncommitted published-Apple-MIG and overlay-minimization candidate reduces the BCR overlay manifest to 15 files; its release pin is intentionally unchanged until the dependency switch is accepted.
...
The macOS Crashpad build generates Mach IPC sources with Apple's `mig` tool. The candidate implementation depends on `apple_mig@138.0.1`, whose standalone module builds `migcom` with `cc_binary`, exposes the upstream `mig.sh` through `sh_binary`, and owns the reviewed generated parser/lexer sources. Sentry Native consumes only the public `@apple_mig//:mig` and `@apple_mig//:migcom` execution tools, so ordinary consumers do not need Bison or Flex.

The Crashpad generator wraps the unchanged upstream `mig.py`, `mig_fix.py`, and `mig_gen.py` in a private rules_python `py_binary` and consumes that launcher as an execution-configured tool. rules_python 1.8 defaults to its system-Python bootstrap, so the execution platform must provide `python3`; consumers whose workers do not may select rules_python's `bootstrap_impl=script` setting to carry the registered interpreter in runfiles. The generator derives Clang from the selected target C++ toolchain, while `mig` and `migcom` are also execution-configured tools. A concrete execroot-relative SDK in that toolchain's compile arguments remains a declared, Linux-remote-capable input. A missing, absolute, or Xcode-placeholder SDK uses `apple_support.run`, whose normal Xcode execution contract provides and resolves `SDKROOT` on a Darwin worker. No action invokes `xcrun` or discovers Clang or MIG from ambient `PATH` state. The Linux execution image must provide `/bin/bash` and the standard utilities used by upstream `mig.sh`; the script is Bash-compatible, not POSIX `sh`.
...
Mitigation: depend on the standalone cross-platform `apple_mig` module, which owns the reproducibly generated parser/lexer C sources and exposes the MIG tools as ordinary Bazel executables. Select all tools through execution configuration. Keep hermetic toolchain SDKs as declared action inputs; use `apple_support.run` only for Xcode-backed SDKs on Darwin execution platforms. Differentially compare both paths' generated Crashpad outputs, cross-build with LLVM on Linux remote workers, and retain native macOS CI for runtime verification.
...
- The released MIG source depends on generated Bison/Flex C files that are not present in Apple's source archive. The standalone `apple_mig` module owns those reviewed generated inputs and their regeneration workflow; Sentry Native no longer needs to overlay or publish them.
- Crashpad's `mig.py`, `mig_gen.py`, and `mig_fix.py` compose cleanly as one rules_python `py_binary`, so the custom MIG rule need not select or invoke a Python interpreter itself. rules_python 1.8 defaults that launcher's `bootstrap_impl` to `system_python`; the generated stage-one launcher begins with `#!/usr/bin/env python3`. This host requirement is accepted for the default graph. Execution platforms without `python3` can opt into `bootstrap_impl=script`, whose launcher uses the registered interpreter from runfiles. Both modes generate all 16 outputs locally, and the script-bootstrap mode also passes on a Linux x86_64 remote worker.
...
- 2026-09-17: Keep Breakpad target declarations explicit in the root package, but move its CMake-derived source/header groups to `bazel/breakpad_sources.bzl` to match the Sentry and Crashpad manifest structure.
- 2026-09-18: Replace the repository-local Apple MIG archive/build/parser overlay with the published BCR `apple_mig@138.0.1` dependency. Keep the established `@apple_mig//:mig` and `@apple_mig//:migcom` labels so the Crashpad rule remains unchanged. Root and isolated-consumer resolution must use the public registry without an `apple_mig` override.
- 2026-09-18: Treat BUILD/Starlark metadata separately from implementation code when minimizing the BCR overlay. Large C++ BUILD overlays are permitted by current BCR policy when their derivation is documented. Eliminate copied/generated implementation bodies where an upstream template or a separate module owns them, and keep registry-only tests to compact consumer-facing fixtures.
- 2026-09-18: Wrap Crashpad's three upstream MIG Python files in a private execution-configured `py_binary` instead of selecting the rules_python interpreter inside `crashpad_mig`. Accept rules_python's default system-`python3` bootstrap; consumers without it can opt into the script bootstrap. Keep the custom rule focused on the target SDK/Clang and execution MIG boundary.
- 2026-09-18: A native `genrule` can replace `crashpad_mig`: list `@bazel_tools//tools/cpp:current_cc_toolchain` in both `srcs` and `toolchains`, use its `$(CC)` template variable, and select the declared SDK directory by its `usr/include/mach/mach_exc.defs` marker. Skylib `select_file` can instead project the LLVM `clang` and `sysroot` files for `run_binary`, but Skylib 1.9.2 `run_binary` has no auxiliary exec-configured `tools` attribute; a two-stage probe reached `mig.sh` but could not materialize the execution-built `migcom`, and placing `migcom` in `srcs` would incorrectly build it for the macOS target during Linux cross-execution. `bazel_lib` 3.3.1's richer `run_binary` does have exec-configured auxiliary tools and could close that gap, at the cost of a new production dependency and include-tree staging. Retain the focused custom rule for this release: both alternatives add more dependency, action, or runtime machinery than they remove.
- 2026-09-18: Support both hermetic and Xcode-backed macOS C++ toolchains in `crashpad_mig`. `CcToolchainInfo.sysroot` is `None` for the configured LLVM 0.8.21 toolchain because rules_cc's `cc_sysroot` models the SDK through compile arguments and data. A concrete execroot-relative compile-action sysroot selects the ordinary remote-capable action; a missing, absolute, or `__BAZEL_XCODE_SDKROOT__` sysroot selects `apple_support.run`, which resolves the SDK placeholder and applies Xcode/Darwin execution requirements. Do not constrain the entire target to macOS execution, because LLVM must retain Linux cross-execution.
...
- MIG generation has two execution contracts. With LLVM, four ordinary actions declare the launcher and runfiles, toolchain, SDK, definitions, and compatibility headers as inputs and remain Linux-remote-capable. With the default Apple C++ toolchain, four `apple_support.run` actions resolve the SDK placeholder under the selected Xcode configuration and carry Darwin/Xcode execution requirements. Both execute the same private `py_binary`, upstream `mig.sh`, and source-built `migcom`; neither invokes `xcrun`. The default launcher intentionally requires execution-platform `python3`; rules_python's script bootstrap makes the LLVM remote graph hermetic when requested by the consumer.
- The standalone `apple_mig@138.0.1` module owns the checked-in parser/lexer inputs and source-built `migcom`. Sentry Native's Crashpad graph consumes its public tools without an owner-module back-reference or repository-local generated sources.
- Linux x86_64 remote workers cross-build the complete `//:sentry` and `//:crashpad_handler` graphs for macOS arm64 and x86_64. After the `py_binary` simplification, the focused external-module code-generation target also passes remotely with rules_python's script bootstrap. Action inspection confirms the `CrashpadMig` actions execute the launcher and its three scripts alongside Linux-built MIG tools and target `@llvm` Clang/SDK. The complete-graph outputs were inspected as arm64 and x86_64 Mach-O executables respectively.
...
- Small BCR publication inputs under `.bcr/`: a metadata template plus versioned presubmit configuration, candidate 15-file manifest, and immutable release descriptor. The descriptor still pins the last committed 49-file overlay during this no-commit validation. The complete module entry is generated only in an official BCR checkout.
...
- In the standalone-module candidate, Apple MIG BUILD files, compatibility headers, parser/lexer outputs, and regeneration workflow are all owned by `apple_mig`; none remain in the Sentry Native overlay.
...
- Before the standalone-module extraction, the official integrity updater and validation tool accepted the repository-local Apple MIG overlay, and `setup_presubmit_repos` rematerialized it successfully. That historical validation is not evidence for the new dependency boundary; focused validation for the candidate is recorded below.
...
- Action-query of the materialized module reports four `CrashpadMig` actions on `@llvm//:rbe_linux_x86_64`, invoking the Crashpad `py_binary`, Linux-built Apple MIG tools, and the selected macOS `@llvm` Clang/sysroot. A focused external-module rerun after the launcher simplification generated all 16 outputs remotely with rules_python's script bootstrap. This verifies the archive-plus-overlay dependency boundary rather than only a root-package action.
...
- Retained only compact, versioned inputs under `.bcr/releases/0.16.6`: the release descriptor, candidate 15-file manifest, and presubmit configuration. During this no-commit validation, the descriptor intentionally continues to pin the previous 49-file overlay commit.
...
- `tools/bcr/verify_release.sh 0.16.6` is intentionally deferred until the candidate is committed and the release descriptor can be repinned; no full BCR validation was requested for this experiment.
...
### Breakpad source-manifest extraction report

Implemented:

- Added `bazel/breakpad_sources.bzl` with the exact common, Linux, Apple/macOS, client, and header groups derived from `external/CMakeLists.txt`.
- Kept `breakpad_client`, its platform selection, LSS ownership, Objective-C++ adapter, and compatibility constraints explicit in the root `BUILD.bazel`.
- Added the manifest to the BCR overlay, returning the release overlay from 48 to 49 files.

Verification:

- Buildifier and root-package loading pass.
- A normalized multiset comparison between the former inline block and the extracted manifest has no source/header differences.
- Per the cleanup policy, no compile or runtime tests were repeated.

### Published Apple MIG module integration

Candidate changes, intentionally uncommitted:

- Replaced the private `http_archive` plus injected BUILD file with the published `bazel_dep(name = "apple_mig", version = "138.0.1")`.
- Removed the repository-local Apple MIG BUILD files, generated parser/lexer sources, and regeneration script; the BCR overlay candidate drops from 49 to 43 files.
- Kept the Crashpad tool labels unchanged as `@apple_mig//:mig` and `@apple_mig//:migcom`.
- Removed the temporary root and isolated-consumer overrides. Both lockfiles now record the public BCR module and source metadata.

Focused verification:

- Both root and isolated-consumer module graphs resolve `apple_mig@138.0.1` from the public BCR.
- The published module exposes the expected public `mig` and `migcom` targets and owns `migcom.tproj/lexxer.c`, `parser.c`, and `parser.h` internally.
- Bazel 9.2.0 builds `@apple_mig//:mig`, `@apple_mig//:migcom`, and all four `CrashpadMig` actions/16 generated outputs from `//:_crashpad_mig_codegen` for LLVM macOS arm64.
- Action-query shows each `CrashpadMig` action consuming `../apple_mig+/migcom.tproj/mig.sh` and execution-built `bazel-out/apple_mig+/.../mig` and `migcom`, with no `//bazel/apple_mig` input.
- From the isolated consumer module, Bazel 9.2.0 regenerates the interfaces and links `@sentry_native//:crashpad_handler` successfully for LLVM macOS arm64.
- After wrapping the Crashpad scripts in `py_binary`, Bazel 9.2.0 builds the full root `//:sentry` and `//:crashpad_handler` macOS arm64 graphs with the default system-Python bootstrap.
- From the isolated consumer, Bazel 9.2.0 generates all 16 MIG outputs on a Linux x86_64 remote worker with rules_python's script bootstrap. The root-package remote attempt hit the already-documented sibling-layout `_main` input-tree failure before reaching MIG; the external-module topology is the relevant consumer result. No full BCR validation or runtime tests were run.
- A temporary standard `genrule` prototype generated the four `child_port` outputs locally and from the isolated consumer on a Linux x86_64 remote worker. Both output sets were byte-identical to the corresponding `crashpad_mig` outputs. The prototype was removed after validation; no replacement has yet been applied.
- Configured-provider inspection proves LLVM 0.8.21's macOS `CcToolchainInfo.sysroot` is `None`; its rules_cc `cc_sysroot` instead contributes concrete SDK paths and data to configured compile actions. The implementation therefore dispatches on that configured action path, not on `.sysroot`.
- A fresh external consumer with no LLVM dependency or registration built and ran a full `sentry` smoke binary with Bazel 9.2.0 using the default Apple C++ toolchain. Bazel 8.4.2 separately regenerated all 16 MIG outputs through the same Xcode-backed path.
- Action-query for the default Apple toolchain shows a placeholder processor for each `CrashpadMig` action, `__BAZEL_XCODE_SDKROOT__` in the SDK/definition arguments, the host macOS execution platform, and `requires-darwin` plus Xcode execution requirements. Action-query for LLVM shows no processor, placeholder, or Darwin requirement.
- The default-Apple and LLVM macOS arm64 output directories are byte-identical across all 16 generated files.
- After the dual-path change, the external consumer regenerated the LLVM macOS arm64 interfaces on a BuildBuddy Linux x86_64 execution platform with rules_python's script bootstrap (invocation `b0e2fa5d-ee0e-4cd6-96b0-e0eba2ca9c09`). Action-query confirms all four actions remain ordinary `CrashpadMig` actions on `rbe_linux_x86_64` with the declared LLVM SDK.

### Overlay-size audit

Initial post-Apple-MIG candidate: 43 files and 202,223 bytes total.

- 23 BUILD, MODULE, `.bzl`, and root-development metadata files: 143,010 bytes. These express the native rules_cc graph and exact CMake-derived source manifests. They are legitimate overlay content; current BCR policy asks that large C++ BUILD overlays document how they were derived rather than forbidding them.
- Three initial build shims: 806 bytes. The Breakpad CoreServices compatibility header and curl runtime anchor are required compatibility shims. The standalone Crashpad client link test duplicated the deeper fork-only Crashpad tests and was removed with its 99-byte source.
- Two libunwind configuration headers: 16,424 bytes. `config.h` is a compact 2,131-byte target contract. The 14,293-byte `libunwind-common.h` was an avoidable configured copy: vendored libunwind already provides the exact content in `cmake/libunwind-common.h.cmake.in`. Materialize that upstream file at the required include name with Skylib `expand_template`.
- Fifteen C/C++ test sources: 41,983 bytes. Together with eight test BUILD files, `tests/**` contributes 23 files and 52,568 bytes. Fourteen of those files are not referenced by BCR presubmit at all and should be omitted from the publication manifest immediately. The remaining Breakpad, Crashpad, and native crash-artifact fixtures are valuable fork validation but are too implementation-heavy for the registry overlay; move their execution to fork CI, remove their BCR presubmit tasks, and retain only the compact `e2e/bcr` consumer module in the published overlay.
- `.bazelrc` and `.bazelignore` are root-checkout conveniences and are not read when sentry-native is an external module or when `e2e/bcr` is the BCR test-module root. Keep them in the fork, but omit them from the publication manifest.

Implemented end state: 15 overlay files and 134,137 bytes after omitting root-only rc/ignore files, all `tests/**` files, the redundant Crashpad link-only test, and the fork-test-only `adjacent_executable.bzl`, and after generating `libunwind-common.h` from the upstream template. The remaining overlay is BUILD/Starlark metadata, two tiny compatibility shims, and the three-file BCR consumer test module. Deep artifact/runtime coverage is configured in a single fork CI lane; BCR presubmit retains public-target builds and compact external-consumer tests. The large BUILD and source-manifest files document their CMake derivation in overlay-visible comments, matching current BCR review guidance.

Final composition: 11 build-metadata files (130,833 bytes), two compatibility shims (707 bytes), one libunwind configuration header (2,131 bytes), and one external-consumer smoke source (466 bytes). No copied third-party implementation or generated parser source remains.

Current BCR precedent confirms that upstream-archive-plus-BUILD-overlay is an established publication model. On registry `main`, 11 distinct modules have a version with at least 10 overlaid BUILD files. Representative official-upstream-source entries include Folly 2026.08.31.00 (95 BUILD files), Boost 1.89.0.bcr.1 (120), Git 2.55.0 (21), Icarus Verilog 13.0.bcr.1 (19), Exempi 2.6.6 (18), GLib 2.82.2.bcr.11 (11), and libarchive 3.8.1.bcr.3 (10). Sentry Native's candidate contains five BUILD files plus its supporting Starlark metadata, so its remaining overlay structure is ordinary by BCR precedent.

Custom overlay rules are established BCR practice, so the current 144-line `crashpad_mig.bzl` is acceptable, but it is no longer the only viable design. A focused native `genrule` probe imported `TemplateVariableInfo` from `@bazel_tools//tools/cpp:current_cc_toolchain`, used `$(CC)` for the target compiler, declared that target toolchain's files as `srcs`, and selected the SDK directory containing `usr/include/mach/mach_exc.defs`. It ran the execution-configured Crashpad `py_binary`, `@apple_mig//:mig`, and `@apple_mig//:migcom`; local macOS and external-module Linux-remote executions produced byte-identical outputs to `crashpad_mig`. The replacement can declare all sixteen outputs directly and use explicit source/header filegroups instead of `OutputGroupInfo`. Its cost is a Bash action and a structural SDK-directory probe rather than `cc_common` extraction of the actual compile action.

A pure Skylib 1.9.2 `run_binary` composition remains incomplete. `select_file` successfully projects `bin/clang` and `sysroot` from the target-configured C++ toolchain, and `run_binary` expands source and output locations. However, `run_binary` provides only its primary executable in exec configuration. A prototype using `@apple_mig//:mig` as that executable reached `mig.sh`, but its execution-built `migcom` data dependency was not materialized; adding `migcom` through `srcs` would put it in the macOS target configuration and make Linux remote execution invalid. A wrapper executable could rediscover and invoke everything, but merely moves the custom logic from Starlark into an overlay script. The maintained `bazel_lib` fork of `run_binary` adds an exec-configured `tools` attribute and richer expansion, so combining it with `select_file` and a staged include tree is technically possible, but introduces another non-dev module and several helper actions solely to remove this rule. Decision: retain `crashpad_mig.bzl` for publication; do not add `bazel_lib`, a wrapper, or a `genrule` replacement. A future semantic generator rule in `apple_mig` remains a possible cleanup, not a submission blocker.

Focused verification:

- Bazel 9.2.0 cross-builds `//vendor/libunwind:unwind` for LLVM Linux x86_64 after the template change.
- The generated `bazel-bin/vendor/libunwind/bazel_include/libunwind-common.h` is byte-identical to `vendor/libunwind/cmake/libunwind-common.h.cmake.in`.
...
- The first post-publication fork-only backend run exposed a direct-checkout-specific Breakpad include failure: the main repository's top-level `external/` path did not propagate the `_breakpad_lss` `include_prefix` virtual include root, although all anonymous BCR module jobs passed. `_breakpad_lss` now exposes the existing upstream `external/` source directory directly, preserving Breakpad's `third_party/lss/linux_syscall_support.h` include spelling in both repository layouts.
- Fork run `35347095947` passed the corrected direct-checkout graph: Ubuntu and macOS minimal static/shared consumers with Bazel 8.4.2 and 9.2.0, plus Linux Breakpad, Crashpad, and native artifact tests. The macOS/Bazel 9 lane passed on retry after its first attempt received a transient GitHub HTTP 504 while downloading Protobuf. BCR-contribution run `35347095927` and end-to-end integration run `35347095903` also passed.
- The exact generated 15-file artifact updated `bazelbuild/bazel-central-registry#10622` at fork commit `624b22c60ad23ca5c715fb435de1ac1fb17efd5f`. Registry validation build `30541` and BCR presubmit build `41553` passed. All 21 reported checks are green, with no pending or failing check.
- Automated publication work is complete. The pull request remains open only for the BCR's required maintainer review and merge.
